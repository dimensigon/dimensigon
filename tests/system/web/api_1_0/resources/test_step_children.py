...
class Test(TestResourceBase):
    ...
    def fill_database(self):
        ...
        self.st2 = self.o.add_step(undo=False, action_template=self.at, children=[self.st1])
        self.st3 = self.o.add_step(undo=False, action_template=self.at, children=[self.st2])
    ...
    def test_get(self):
        resp = self.client.get(url_for('api_1_0.steprelationshipchildren', step_id=str(self.st1.id)),
                               headers=self.auth.header)

        self.assertEqual(200, resp.status_code)
        self.assertDictEqual(dict(child_step_ids=[]), resp.get_json())

        resp = self.client.get(url_for('api_1_0.steprelationshipchildren', step_id=str(self.st2.id)),
                               headers=self.auth.header)

        self.assertEqual(200, resp.status_code)
        self.assertDictEqual(dict(child_step_ids=[str(self.st1.id)]), resp.get_json())

    def test_patch(self):
        resp = self.client.patch(url_for('api_1_0.steprelationshipchildren', step_id=str(self.st3.id)),
    ...
    def test_post(self):
        resp = self.client.post(url_for('api_1_0.steprelationshipchildren', step_id=str(self.st3.id)),
                                json={'child_step_ids': [str(self.st1.id)]},
                                headers=self.auth.header)
    ...
    def test_delete(self):
        resp = self.client.delete(url_for('api_1_0.steprelationshipchildren', step_id=str(self.st3.id)),
