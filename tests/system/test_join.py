...
class TestApi(TestCase):
    ...
    def test_join_command(self, mock_rsa):
        ...
        with responses.RequestsMock() as rsps:
            ...
            runner = CliRunner(env={'FLASK_CONFIG': 'test'})
            with patch('dimensigon.db.session.add') as mock_add:
                with patch('dimensigon.db.session.commit') as mock_commit:
                    result = runner.invoke(cli, ['dm', 'join', '--no-ssl', 'server:5000', create_access_token('join')],
                                           catch_exceptions=False, env={'FLASK_CONFIG': 'test'})
                    mock_add.assert_called_once()

                    self.assertEqual(self.dimension, mock_add.call_args[0][0].to_json())
