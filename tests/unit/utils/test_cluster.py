...
from dimensigon.utils.cluster import ClusterManager, _ClusterRegister, ClusterManagerCoordinator, \
    date_format
...
class TestClusterManager(TestCase):
    ...
    def test_set_keepalive(self, mock_now):
        ...
        self.cm.threshold = dt.timedelta(seconds=1)
        self.cm.set_keepalive(2, after_now)
        self.assertDictEqual({1: _ClusterRegister(1, birth=before_now, keepalive=now, death=None),
                              2: _ClusterRegister(2, birth=now, keepalive=now, death=None)},
                             self.cm._cluster)
...
#                              self.cm._cluster)
#
#         self.assertIsNone(self.cm.set_death(1, 12, after_now))
#
#     @mock.patch('dimensigon.utils.cluster_manager.get_now')
#     def test_set_keepalive(self, mock_now):
#         mock_now.return_value = now
