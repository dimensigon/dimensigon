...
class TestInMemoryDao(TestCase):
    ...
    def test_filtered_get_fail(self):
        assert self.dao.filter(pred_not_a).get(1) is None
    ...
    def test_remove_filtered(self):
        ...
        assert ids == [1]
        assert get_ids(self.dao.all()) == [2, 3]
    ...
    def test_insert(self):
        id_ = self.dao.insert(dict(foo='bar'))
        assert id_ == 4

    def test_insert_with_id(self):
        id_ = self.dao.insert(dict(foo='bar', id=4))
        assert id_ == 4
    ...
    def test_batch_insert(self):
        ...
        assert result == (4, 5)
        self.assertListEqual([{'foo': 'bar', 'id': 4}, {'foo': 'baz', 'id': 5}],
                             [dict(dto) for dto in self.dao.filter(where('foo').exists())])
