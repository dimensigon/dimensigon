...
class TestPack_msg(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.token = Fernet.generate_key()

    def test_pack_unpack_msg_with_keys(self):
        packed_msg = pack_msg(self.dest, self.source, self.pub_key, self.priv_key, data=self.data)
    ...
    def test_pack_unpack_msg_without_keys(self):
        packed_msg = pack_msg(self.dest, self.source, data=self.data)
    ...
    def test_pack_unpack_with_symmetric_key(self):
        packed_msg = pack_msg(self.dest, self.source, self.pub_key, self.priv_key, symmetric_key=self.token,
                              data=self.data)
        self.assertIn('source', packed_msg)
        self.assertIn('destination', packed_msg)
        self.assertNotIn('key', packed_msg)
    ...
    def test_pack_unpack_with_symmetric_key_no_rsa_keys(self):
        packed_msg = pack_msg(self.dest, self.source, symmetric_key=self.token,
                              data=self.data)
        self.assertIn('source', packed_msg)
        self.assertIn('destination', packed_msg)
        self.assertNotIn('key', packed_msg)
    ...
    def test_pack_unpack_with_symmetric_key_encrypted(self):
        sym_key_encrypted = rsa.encrypt(self.token, self.pub_key)
        packed_msg = pack_msg(self.dest, self.source, self.pub_key, self.priv_key, cipher_key=sym_key_encrypted,
                              data=self.data)
        self.assertIn('source', packed_msg)
        self.assertIn('destination', packed_msg)
        self.assertNotIn('key', packed_msg)
        ...
        unpacked_msg = unpack_msg(packed_msg, pub_key=self.pub_key, priv_key=self.priv_key, cipher_key=sym_key_encrypted)
    ...
    def test_pack_error(self):
        ...
        with self.assertRaises(ValueError) as e:
            pack_msg(self.dest, self.source, pub_key=self.pub_key, cipher_key=sym_key_encrypted, data=self.data)
    ...
    def test_unpack_with_symmetric_key_as_parameter_and_cipher_in_msg(self, mocked_generate_key):
        mocked_generate_key.return_value = self.token
        sym_key_encrypted = rsa.encrypt(self.token, self.pub_key)
        packed_msg = pack_msg(self.dest, self.source, self.pub_key, self.priv_key, data=self.data)
        self.assertIn('source', packed_msg)
        self.assertIn('destination', packed_msg)
