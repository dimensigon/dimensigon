...
def join(server, token):
    ...
    print('Request')
    resp = requests.post(f"https://{server}/api/join", json=data)
    print('Raise for status')
    resp.raise_for_status()
    resp_data = unpack_msg(resp.json, pub_key=d_pub_key, priv_key=priv)
    repo_manager.DimensionRepo.add(resp_data)
    print('Joined')
...
def new(name):
    ...
    d = repo_manager.DimensionRepo.create_and_add({'name': name, 'pub': pub, 'priv': priv})
    click.echo('The UUID for the dimension is %s' % d.id)
